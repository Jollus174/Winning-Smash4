var Page = (function(){
	...
	return {
		...
		// Run the custom JS code after everything is binded and dandy

		runCustomJS: function(){
			console.log('now running the custom js!');
		    ...
		    // });
		    // This won't work yet, because 'disabled' may not be a class at run-time, and is dynamically added
		    var _characterbox = document.querySelectorAll('.character-box:not(.disabled)');

		    Array.prototype.forEach.call(_characterbox, function(el, i){
		    	if(el.classList){
		    ...
		    // Did I still need this custom event??
		    ...
		    function activateCharacter(self){
		      ...
		      var name = self.getAttribute('data-name'),
		        urlName = self.getAttribute('data-url'),
		        bgColour = self.getAttribute('data-bgcolour'),
		        weight = parseInt(self.getAttribute('data-weight')),
		        fallspeed = self.getAttribute('data-fallspeed'),
		        gravity = self.getAttribute('data-gravity'),
		        airdodgeStart = parseInt(self.getAttribute('data-airdodgestart')),
		        airdodgeEnd = parseInt(self.getAttribute('data-airdodgeend')),
		        minPercent = parseInt(self.getAttribute('data-minpercent')),
		        maxPercent = parseInt(self.getAttribute('data-maxpercent')),
		        textContrast = self.getAttribute('data-textcontrast'),
