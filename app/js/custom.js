...
var Custom = (function () {
	...
	var $body = $('body'),
		$charContainer = $('#characterModal .characterContainer');
	...
	function rageAdjustment(self, animateNumbers) {
		...
		if (rageAmount === '100') {
			rageAdjMin = rage100[0];
			rageAdjMax = rage100[1];
		}
		if (rageAmount === '125') {
			rageAdjMin = rage125[0];
			rageAdjMax = rage125[1];
		}
		if (rageAmount === '150') {
			rageAdjMin = rage150[0];
			rageAdjMax = rage150[1];
		}
		...
		$('#characterModal .stage .table').each(function () {
			...
			if (adjustedMaxPercent < adjustedMinPercent) {
				...
				if (animateNumbers === true) {
					$minPerc.prop('number', $minPerc.text()).animateNumber({ number: adjustedMinPercent }, 200);
					$maxPerc.prop('number', $maxPerc.text()).animateNumber({ number: adjustedMaxPercent }, 200);
					$percRange.prop('number', $percRange.text()).animateNumber({ number: percRange }, 200);
				} else {
	...
	function activateCharacter(self) {
		...
		$bf
			.find('span[data-ref="bfNormalMin"]')
			.text(minPercent + bfNormalMin)
			.attr('data-defaultmin', minPercent + bfNormalMin);
		$bf.find('span[data-ref="bfNormalMax"]').text(maxPercent).attr('data-defaultmax', maxPercent);
		$bf
			.find('span[data-ref="bfLowPlatMin"]')
			.text(minPercent + bfLowPlatMin)
			.attr('data-defaultmin', minPercent + bfLowPlatMin);
		$bf.find('span[data-ref="bfLowPlatMax"]').text(maxPercent).attr('data-defaultmax', maxPercent);
		$bf
			.find('span[data-ref="bfTopPlatMin"]')
			.text(minPercent + bfTopPlatMin)
			.attr('data-defaultmin', minPercent + bfTopPlatMin);
		...
		$dl
			.find('span[data-ref="dlLowPlatMin"]')
			.text(minPercent + dlLowPlatMin)
			.attr('data-defaultmin', minPercent + dlLowPlatMin);
		$dl.find('span[data-ref="dlLowPlatMax"]').text(maxPercent).attr('data-defaultmax', maxPercent);
		$dl
			.find('span[data-ref="dlTopPlatMin"]')
			.text(minPercent + dlTopPlatMin)
			.attr('data-defaultmin', minPercent + dlTopPlatMin);
		$dl.find('span[data-ref="dlTopPlatMax"]').text(maxPercent).attr('data-defaultmax', maxPercent);

		var $sv = $charModal.find('.stage-sv');
		$sv
			.find('span[data-ref="svNormalMin"]')
			.text(minPercent + svNormalMin)
			.attr('data-defaultmin', minPercent + svNormalMin);
		$sv.find('span[data-ref="svNormalMax"]').text(maxPercent).attr('data-defaultmax', maxPercent);
		$sv
			.find('span[data-ref="svPlatMin"]')
			.text(minPercent + svPlatMin)
			.attr('data-defaultmin', minPercent + svPlatMin);
		$sv.find('span[data-ref="svPlatMax"]').text(maxPercent).attr('data-defaultmax', maxPercent);

		var $tc = $charModal.find('.stage-tc');
		$tc
			.find('span[data-ref="tcNormalMin"]')
			.text(minPercent + tcNormalMin)
			.attr('data-defaultmin', minPercent + tcNormalMin);
		$tc.find('span[data-ref="tcNormalMax"]').text(maxPercent).attr('data-defaultmax', maxPercent);
		$tc
			.find('span[data-ref="tcLowPlatMin"]')
			.text(minPercent + tcLowPlatMin)
			.attr('data-defaultmin', minPercent + tcLowPlatMin);
		$tc.find('span[data-ref="tcLowPlatMax"]').text(maxPercent).attr('data-defaultmax', maxPercent);
		$tc
			.find('span[data-ref="tcSidePlatMin"]')
			.text(minPercent + tcSidePlatMin)
			.attr('data-defaultmin', minPercent + tcSidePlatMin);
		$tc.find('span[data-ref="tcSidePlatMax"]').text(maxPercent).attr('data-defaultmax', maxPercent);
		$tc
			.find('span[data-ref="tcTopPlatMin"]')
			.text(minPercent + tcTopPlatMin)
			.attr('data-defaultmin', minPercent + tcTopPlatMin);
		...
		if (!$this.hasClass('active')) {
			...
			$.each(killConfirmsJSON[characterIndex].moves[moveIndex].percents, function (index, value) {
				...
				if (moveUrl !== 'zelda-dthrow-up-air') {
					...
					if (0 <= percentDiff && percentDiff <= diffIterator) {
						diffClass = 'very-hard';
					} else if (diffIterator <= percentDiff && percentDiff <= diffIterator * 2) {
						diffClass = 'hard';
					} else if (diffIterator * 2 <= percentDiff && percentDiff <= diffIterator * 3) {
						diffClass = 'average';
					} else if (diffIterator * 3 <= percentDiff && percentDiff <= diffIterator * 4) {
						diffClass = 'easy';
					} else if (diffIterator * 4 <= percentDiff) {
						diffClass = 'very-easy';
					} else {
						diffClass = '';
					}
					...
					if (airdodgeStart === 1) {
						diffClass = 'very-hard';
					} else if (airdodgeStart === 2) {
						diffClass = 'hard';
					} else if (airdodgeStart === 3) {
						diffClass = 'average';
					} else if (airdodgeStart === 4) {
						diffClass = 'easy';
					} else {
						diffClass = '';
					}
	...
	function pageTransition(self, transitionToAnotherGrid) {
		...
		if (transitionToAnotherGrid) {
			...
			$nonCurrPage
				.removeClass()
				.addClass('pt-page pt-page-current')
				.addClass(inClass)
		...
		if (debug.debugMode && debug.transitionType.length) {
			console.log('transition animation: ' + debug.transitionType);
		}
	}
					...
					if (!parts[2]) {
						...
						if ($('body').hasClass('character-grid-active')) {
							...
							if (parts[1] !== $('.moveBtn.active').attr('id')) {
								...
								// transitionToAnotherGrid = true for the below
								pageTransition($('#' + parts[1]), true);
							} else {
	...
	function transitionRageForward($activeRageButton) {
		if (!$activeRageButton.is(':last-of-type')) {
			// animateNumbers = true for the below
			rageAdjustment($activeRageButton.next(), true);
		} else {
			rageAdjustment($('.rageModifier .rageBtn:first-of-type'), true);
		}
	}
	function transitionRageBackward($activeRageButton) {
		if (!$activeRageButton.is(':first-of-type')) {
			rageAdjustment($activeRageButton.prev(), true);
		} else {
			rageAdjustment($activeRageButton.siblings('.rageBtn:last-of-type'), true);
		}
	}
	...
	$(document)
		...
		.keydown(function (e) {
			...
			isShiftActive = e.which === keyShift;
			isAltActive = e.which === keyAlt;
			...
			if ($('#characterModal.active').length && !isAltActive) {
				var $this = $(this);
				...
				if (isShiftActive) {
					if (e.which === keyright) {
						...
						transitionRageForward($this.find('.rageBtn.active'), true);
					}
					if (e.which === keyleft) {
						//console.log('shift held and left');
						transitionRageBackward($this.find('.rageBtn.active'), true);
					}
				} else {
					if (e.which === keyright) {
					...
					if (e.which === key1) {
						rageAdjustment($this.find('.rageBtn:nth-child(1)', true));
					}
					if (e.which === key2) {
						rageAdjustment($this.find('.rageBtn:nth-child(2)', true));
					}
					if (e.which === key3) {
						rageAdjustment($this.find('.rageBtn:nth-child(3)', true));
					}
					if (e.which === key4) {
						rageAdjustment($this.find('.rageBtn:nth-child(4)', true));
					}
					if (e.which === key5) {
						rageAdjustment($this.find('.rageBtn:nth-child(5)', true));
					}
					if (e.which === key6) {
						rageAdjustment($this.find('.rageBtn:nth-child(6)', true));
					}
					if (e.which === key7) {
						rageAdjustment($this.find('.rageBtn:nth-child(7)', true));
					}
				}
			}
		});
